...
def centroidSearch(start):
    ...
    #look for first frontier
    while (not done and not rospy.is_shutdown()):
       # print "checking cell"
        for next in CurrGrid.allNeighbors(start):
            try:
                visIndex = visited.index(next)
            except ValueError:
                visIndex = -1
            ...
            if(visIndex == -1):
               ...
               # print next
                ...
                startPoint = Point()
                startPoint.x = (start[0]*res) + origin.x + .025
                startPoint.y = (start[1]*res) + origin.y + .025
                startPoint.z=0

                if (startPoint in f_cells):
    ...
    while (not done and not rospy.is_shutdown()):
        ...
        for next in CurrGrid.unkNeighbors(unknown):
            ...
            if(visIndex >= 0 and unexplored(next)):
                print "adding to visited"
                visited.append(next)
                toSearch2.put(next)
